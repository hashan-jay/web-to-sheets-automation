...
If the site uses 2FA, set `MANUAL_LOGIN_SECONDS=90` and complete the prompt in the opened browser on the first run. The login session is saved to `auth_state.json`. Later runs reuse that session and read the Completed list over HTTP. **Automated Run** keeps Completed live: new IDs appear in the GUI and on the Google Sheet within 1–2 seconds.
...
| Deposit | day | datetime | *(blank, user selects)* | player name | amount | Deposit | ID | brand | | username | | *(blank, user selects)* |
...
Column G is the duplicate key. Existing IDs are skipped. Withdrawal amounts are stored as negatives so the sheet can sum them. Company TRF, Staff, and Bank are left blank for the dropdowns.
